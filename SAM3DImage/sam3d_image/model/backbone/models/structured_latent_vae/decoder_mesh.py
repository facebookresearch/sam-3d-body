...
class SLatMeshDecoder(SparseTransformerBase):
    def __init__(
        ...
        if num_upsample_layers > 2:
            for i in range(num_upsample_layers - 2):
                upsample_module_list.append(
                    SparseSubdivideBlock3d(
                        channels=model_channels // 8,
                        resolution=resolution * 2 ** (i + 2),
                        out_channels=model_channels // 8,
                    )
                )

        self.upsample = nn.ModuleList(upsample_module_list)
