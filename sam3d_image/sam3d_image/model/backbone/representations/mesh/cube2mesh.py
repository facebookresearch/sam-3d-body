...
class SparseFeatures2Mesh:
    def __init__(self, device="cuda", res=64, use_color=True):
        ...
        self.device = device
        self.res = res
        # defer to first-call for lazy init
    ...
    def mesh_extractor(self):
        # lazy init, init at first call when device is setup properly
        if self._mesh_extractor is None:
            self._mesh_extractor = FlexiCubes(device=self.device)
    ...
    def __call__(self, cubefeats: SparseTensor, training=False):
        ...
        self._reset_device(feats.device)
