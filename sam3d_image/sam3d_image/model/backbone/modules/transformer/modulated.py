...
class MOTModulatedTransformerCrossBlock(nn.Module):
    ...
    # This is stupid, optree does not support ModuleDict
    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor):
        ...
        h = optree.tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm1))
        if self.safe_fp16:
            h = optree.tree_map(
                partial(self._cast_type, dtype=torch.float16),
                h,
            )
        h = optree.tree_map(
        ...
        h = self.self_attn(h)
        h = optree.tree_map(partial(self._apply_multiplication, multiplier=gate_msa), h)
        if self.safe_fp16:
            h = optree.tree_map(
                partial(self._cast_type, dtype=torch.float32),
                h,
            )
        x = optree.tree_map(self._apply_add, x, h)
        h = optree.tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm2))
        if self.safe_fp16:
            h = optree.tree_map(
                partial(self._cast_type, dtype=torch.float16),
                h,
        ...
        x = optree.tree_map(self._apply_add, x, h)
        h = optree.tree_map(self._apply_module, x, self._moduledict_to_dict(self.norm3))
        if self.safe_fp16:
            h = optree.tree_map(
                partial(self._cast_type, dtype=torch.float16),
                h,
        ...
        h = optree.tree_map(self._apply_module, h, self._moduledict_to_dict(self.mlp))
        h = optree.tree_map(partial(self._apply_multiplication, multiplier=gate_mlp), h)
        if self.safe_fp16:
            h = optree.tree_map(
                partial(self._cast_type, dtype=torch.float32),
                h,
