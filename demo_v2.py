...
def main(args):
    ...
    for image_path in tqdm(images_list):
        ...
        # Run human detection
        boxes = human_detector.run_human_detection(
            img,
            det_cat_id=0,
            bbox_thr=0.5,
            nms_thr=0.3,
        ...
        # NOTE: The following models expect RGB images
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        
        # Get Moge intrinsics estimation
        cam_int = fov_estimator.get_cam_intrinsics(img)

        # Get SAM masks
        masks = None
        if args.use_mask:
            masks, masks_score = human_segmentor.run_sam(img, boxes)
