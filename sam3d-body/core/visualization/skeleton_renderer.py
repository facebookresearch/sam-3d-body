...
#        num_samples = pred_keypoints_3d.shape[1]
        ...
        if camera_translation is None:
            camera_translation = torch.tensor([0.0, 0.0, 2 * self.cfg.MODEL.SMPL_HEAD.FOCAL_LENGTH / (0.8 * self.cfg.MODEL.IMAGE_SIZE)]).unsqueeze(0).repeat(batch_size, 1)
        else:
        ...
        focal_length = torch.tensor([self.cfg.MODEL.SMPL_HEAD.FOCAL_LENGTH, self.cfg.MODEL.SMPL_HEAD.FOCAL_LENGTH]).reshape(1, 2)
