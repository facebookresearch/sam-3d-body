...
from core.models.modules import to_2tuple
...
class ResizePadFull(nn.Module):
    ...
    def forward(self, results: Dict) -> Optional[dict]:
        # NOTE: image normalization does not happen here but in `_prepare_batch` in the model
        orig_center, orig_scale = results["bbox_center"], results["bbox_scale"]
        orig_bbox = bbox_cs2xyxy(orig_center, orig_scale)
        results["boxes"] = orig_bbox
        # results['keypoints_2d'] = results.get('keypoints_2d', None)

        img = results["img"]
        size = np.array([img.shape[1], img.shape[0]])
        scale = self.IMG_SIZE[0] / max(size)
        offset = (self.IMG_SIZE[0] - scale * size) / 2
        ...
        img_pil = ImageOps.contain(img_pil, self.IMG_SIZE)
        img_pil = ImageOps.pad(img_pil, size=self.IMG_SIZE)

        # if self.zoom is not None:
        #     assert type(self.zoom) == float
        #     zooming = int(self.zoom * self.IMG_SIZE[0])
        #     zooming = CenterCrop(zooming)
        #     img_pil = ImageOps.contain(zooming(img_pil), self.IMG_SIZE)
        # else:
        #     zoom = 1.0
        ...
        if results.get("keypoints_2d", None) is not None:
            results["orig_keypoints_2d"] = results["keypoints_2d"].copy()
        results["img"] = img
        results["cam_int"][:2] *= scale
        results["cam_int"][:2, -1] += offset
        # results["cam_int"][:2] /= zoom

        if results["boxes"] is not None:
            results["boxes"][:, :4] *= scale
            results["boxes"][:, :2] += offset
            results["boxes"][:, 2:4] += offset

            # zoom_offset = (self.IMG_SIZE[0] - int(zoom * self.IMG_SIZE[0])) / 2
            # results["boxes"][:, :4] -= zoom_offset
            # results["boxes"][:, :4] /= zoom
            results["boxes"][:, :4] = results["boxes"][:, :4].clip(0, self.IMG_SIZE[0])

        # # NOTE: this is how promptHMR does it, but may we don't have to do it for
        # # our eval code, it may introduce error in the metric calculation
        # if results.get("keypoints_2d", None) is not None:
        #     kpts = results["keypoints_2d"].copy()
        #     kpts[:, :, :2] *= scale
        #     kpts[:, :, :2] += offset
        #     results["keypoints_2d"] = kpts

        results["image_scale"] = scale
        results["image_offset"] = offset
        # results["image_zoom"] = zoom

        return results


class ResizePadFullv2(nn.Module):
    """
    Resize and pad the image to the target size.
    Required Keys:
        - keypoints_2d
    ...
    def forward(self, results: Dict) -> Optional[dict]:
        # NOTE: image normalization does not happen here 
        # but in `_prepare_batch` in the model
